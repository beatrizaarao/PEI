...
//array ao ano
select MONTH(BEGIN_DATE),count(ID_ORDER) from ORDEM WHERE YEAR(BEGIN_DATE) > YEAR(CURRENT_DATE)-1 and MONTH(BEGIN_DATE) > MONTH(CURRENT_DATE) group BY MONTH(BEGIN_DATE) 

//MESMAS ENCOMENDAS ANO ANTTERIOR
SELECT COUNT(ID_ORDER) FROM ORDEM WHERE YEAR(BEGIN_DATE)= YEAR(CURRENT_DATE)-1 and MONTH(BEGIN_DATE) = MONTH(CURRENT_DATE) and DAY(BEGIN_DATE) <= DAY(CURRENT_DATE)
