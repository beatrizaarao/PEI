    ...
    if (req.cookies.deploy === undefined) {
        ...
        db.query("SELECT * FROM ORDER_STEP_SERVICE_OPTION WHERE ID_ORDER=?", req.params.orderID, function (error, result, client) {
                        ...
                        db.query("SELECT NAME FROM SERVICE WHERE id_SERVICE=?", currentServiceID, function (error, result, client) {
                            ...
                            db.query("SELECT NAME FROM OPCAO WHERE id_OPTION=?", currentOptionID, function (error, result, client) {
                                if(result != null){
                                    ...
                                    if(array2== undefined){
                                        if(results[foo].opcao_valor==null){array2 = [optionName];}
                                        else{array2 = [optionName + ": " + results[foo].opcao_valor];}

                                        serviceOption.set(serviceName, array2);
                                    }
                                    else {
                                        if(results[foo].opcao_valor==null){array2.push(optionName);}
                                        else{array2.push(optionName + ": " + results[foo].opcao_valor);}
                                ...
                                if (array == undefined) {
                                    ...
                                    if(array.includes(serviceName)==false){
                                        if(results[foo].SERVICE_valor==null){array.push(serviceName);}
                                        else{array.push(serviceName + ": " + results[foo].SERVICE_valor);}
            ...
            function done() {
                ...
                res.render('orderInfo', { title: 'Informação Encomenda', stepService: stepService, steps: steps, services: services, serviceOption: serviceOption});
            }
